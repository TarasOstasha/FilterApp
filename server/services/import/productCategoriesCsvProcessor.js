...
const fs = require('fs');
const csv = require('csv-parser');
const db = require('../../models/index'); // Your DB config
    ...
    fs.createReadStream(csvFilePath)
      ...
      .on('data', async (row) => {
        try {
          ...
          errorOccurred = true;
          console.error(chalk.red('Error inserting product categories:', err));
        }
      })
      .on('end', () => {
        if (!errorOccurred) {
          console.log(
            chalk.green('CSV file processed successfully and product_categories inserted.')
          );
        }
        resolve(results);
      })
